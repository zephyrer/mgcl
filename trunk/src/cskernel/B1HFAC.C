...
//The original codes of this program comes from the FORTRAN code BCHFAC of
...
//FACTORIZATION . 
    for(n=1; n<=nrow; ++n){
		if(w[n*nbands+1]+diag[n]<=diag[n]){
			for(j=1; j<=nbands; ++j)
				w[j+n*nbands] = 0.f;
			continue;
		}

		w[n*nbands+1] = 1.f/w[n*nbands+1];
		imax = nrow-n;
		if(imax>nbands-1)
			imax = nbands-1;
		if(imax<1)
		...
		jmax = imax;
		for(i=1; i<=imax; ++i){
			ratio = w[i+1+n*nbands]*w[n*nbands+1];
		    for(j=1; j<=jmax; ++j)
				w[j+(n+i)*nbands] -= w[j+i+n*nbands]*ratio;
			--jmax;
		    w[i+1+n*nbands] = ratio;
		}
    }
}
